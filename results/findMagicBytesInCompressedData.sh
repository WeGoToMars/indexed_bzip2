...
while true; do
    ...
    for i in $( seq $parallelism ); do
        ...
        openssl enc -aes-256-ctr -pass pass:"$( dd if=/dev/urandom bs=128 count=1 2>/dev/null | base64 )" -nosalt </dev/zero 2>/dev/null | head -c "$sizePerTest" > "$file" # ~80ms for 128MB
    done
    echo "[$( date +%s )] Created $(( sizePerTest / ( 1024 * 1024 * 1024 ) )) GiB of random data"
    ...
    find . -iname '*.bz2' -print0 | xargs -0 -P $parallelism -I{} bash -c '
        bzcat "$0" "$(( 1024 * 1024 ))" 2>&1 >/dev/null | sed -nE "s|([0.9]+ B [0-9] b).*0x314159.*|\1|p" > "${0%.bz2}".decode.lst
    ' {}
